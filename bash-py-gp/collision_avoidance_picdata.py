...
def MessageSize(typePrefix, directory):
    ...
    wf.write("#MPM100 ratio MPM200 ratio MPM300 ratio MPM400 ratio MPM500 ratio MPM600 ratio NoLimit ratio\n")
    num = 100 # may subject to change by the simulation node number
    for backofftime in [0,0.5,1,1.5,2,2.5,3]:
        wf.write(str(backofftime)+" ")
        for mms in [100,200,300,400,500,600,-1]:
            file = open("%(directory)s%(typePrefix)s_mb%(backofftime)s_mms%(mms)s_overhead.data"%vars())
            for line in file:
                if line[0] != "#":
                    numbers = line.split(' ')
                    wf.write(numbers[7]+" ")
...
def RecallAndLatency(typePrefix, directory):
    ...
    for backofftime in [0,0.5,1,1.5,2,2.5,3]:
        recallf.write(str(backofftime)+" ")
        latencyf.write(str(backofftime)+" ")
        for mms in [100,200,300,400,500,600,-1]:
            file = open("%(directory)s%(typePrefix)s_mb%(backofftime)s_mms%(mms)s_0.data"%vars())
...
if len(sys.argv) <=1:
    ...
    if sys.argv[1] == "a":
        ...
        RecallAndLatency("collision_avoidance", "./")
    elif sys.argv[1] == "e":
        MessageSize("collision_avoidance", "./")
