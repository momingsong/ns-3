...
public:
  uint32_t GetLength() {
    return sizeof(unsigned int) * 2
          ...
          + sizeof(double)
          + sizeof(char) * raw_table_size_
          + sizeof(bloom_type) * salt_count_;
  }

  uint32_t Encode(uint8_t *begin) {
    uint8_t *p = begin;
    *((unsigned int *)p) = salt_count_;
    p += sizeof(unsigned int);
    *((unsigned long long int *)p) = table_size_;
    p += sizeof(unsigned long long int);
    *((unsigned long long int *)p) = raw_table_size_;
    p += sizeof(unsigned long long int);
    *((unsigned long long int *)p) = projected_element_count_;
    p += sizeof(unsigned long long int);
    *((unsigned int *)p) = inserted_element_count_;
    p += sizeof(unsigned int);
    *((unsigned long long int *)p) = random_seed_;
    p += sizeof(unsigned long long int);
    *((double *)p) = desired_false_positive_probability_;
    p += sizeof(double);
    std::copy(bit_table_, bit_table_ + raw_table_size_, p);
    p += raw_table_size_;
    std::vector<bloom_type>::iterator iter = salt_.begin();
    while (iter != salt_.end()) {
      *((bloom_type *)p) = *iter;
      p += sizeof(bloom_type);
      ++iter;
    }
  ...
  uint32_t Decode(uint8_t *begin) {
    uint8_t *p = begin;
    salt_count_ = *((unsigned int *)p);
    p += sizeof(unsigned int);
    table_size_ = *((unsigned long long int *)p);
    p += sizeof(unsigned long long int);
    raw_table_size_ = *((unsigned long long int *)p);
    p += sizeof(unsigned long long int);
    projected_element_count_ = *((unsigned long long int *)p);
    p += sizeof(unsigned long long int);
    inserted_element_count_ = *((unsigned int *)p);
    p += sizeof(unsigned int);
    random_seed_ = *((unsigned long long int *)p);
    p += sizeof(unsigned long long int);
    desired_false_positive_probability_ = *((double *)p);
    p += sizeof(double);
    if (bit_table_ != NULL)
      delete [] bit_table_;
