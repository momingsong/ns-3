...
NS_LOG_COMPONENT_DEFINE("PecMobility");
...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("mobilityTrace", "Path of the mobility trace file", mobility_trace);
  ...
  stream << "# tracePath=" << trace_path << std::endl
         << "# mobilityTrace=" << mobility_trace << std::endl
  ...
  Ns2MobilityHelper mobility = Ns2MobilityHelper (mobility_trace);
  mobility.Install();
  ...
  for (int i = 0; i < consumer_num; ++i) {
    ...
    while (same) {
      ...
      same = false;
      for (unsigned int j = 0; j < consumer_list.size(); ++j) {
        if (consumer_list[j] == consumer_index) {
          same = true;
          break;
        }
      }
